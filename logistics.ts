...
import { EventKnowledgeGraph, EventNode, ModelEntities, ModelRelations } from "./types";

import fs from "fs";
import { discover, findRelationClosures, makeOCLogFromClosure } from "./src/ocDiscovery";
...
const include_entities = ['Customer Order', 'Transport Document', 'Container', 'Truck', 'Handling Unit', 'Forklift', 'Vehicle'];
const model_entities_derived: Array<string> = [];
const subprocess_entities: Array<string> = [];
...
for (const event of json.events) {
    ...
    for (const related of event.relationships) {
        ...
        if (!type) console.log("hehehehehe ", related)
        if (!type || !include_entities.includes(type)) continue;
...
console.log(findTransitiveCardinalities(ekg, findRelationClosures(ekg)));
